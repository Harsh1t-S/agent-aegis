...
1. Create an isolated staging Supabase project, Razorpay test account configuration,
...
6. Verify Razorpay test checkout, renewal, failed payment, plan change and cancellation.
...
- **Billing webhook failure:** retain the signed receipt, fix the handler or
  configuration, and replay the event from Razorpay. Event IDs and provider creation
